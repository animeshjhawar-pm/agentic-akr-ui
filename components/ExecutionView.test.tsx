...
// - 5 tabs render with role=tab (Commentary, Events, Keywords, Clusters, Artifacts)
// - TotalsHeader stays visible across tab switches
// - "Events" tab shows event log section with Linear/Tree toggle
// - Clicking Commentary tab renders Commentary component
// - Clicking Artifacts tab renders Artifacts component
// - Clicking Keywords tab shows Results (or placeholder pre-completion)
// - Clicking Clusters tab shows Clusters component
// - Arrow-key navigation moves focus between tabs (wraps at 5)
...
vi.mock('@/lib/useRunStream', () => ({
  useRunStream: (...args: unknown[]) => mockUseRunStream(...args),
}));
...
vi.mock('./Artifacts', () => ({
  default: ({ runId }: { runId: string }) => (
    <div data-testid="artifacts">Artifacts({runId})</div>
  ),
}));
...
function makeReader(): ReadableStreamDefaultReader<Uint8Array> {
  // A reader that is never read -- stream stays open
...
const DONE_STREAM_STATE = {
  ...BASE_STREAM_STATE,
  streamDone: true,
};

const STALLED_STREAM_STATE = {
  ...BASE_STREAM_STATE,
  stalled: true,
};
...
describe('ExecutionView (tabbed)', () => {
  beforeEach(() => {
    mockUseRunStream.mockReturnValue(BASE_STREAM_STATE);
  });
  ...
  it('renders 5 tabs with role=tab', () => {
    render(<ExecutionView runId="run-1" reader={makeReader()} />);
  ...
  it('TotalsHeader stays visible when switching to Artifacts tab', () => {
    render(<ExecutionView runId="run-1" reader={makeReader()} />);
  ...
  it('TotalsHeader stays visible when switching to Keywords tab', () => {
    mockUseRunStream.mockReturnValue(DONE_STREAM_STATE);
    render(<ExecutionView runId="run-1" reader={makeReader()} />);
  ...
  // ---- Clicking Artifacts tab ----------------------------------------------

  it('clicking Artifacts tab shows Artifacts component', () => {
    render(<ExecutionView runId="run-1" reader={makeReader()} />);
  ...
  it('clicking Artifacts tab hides the events panel', () => {
    render(<ExecutionView runId="run-1" reader={makeReader()} />);
  ...
  it('Artifacts panel passes correct runId', () => {
    render(<ExecutionView runId="run-xyz" reader={makeReader()} />);

    fireEvent.click(screen.getByRole('tab', { name: /^artifacts$/i }));

    expect(screen.getByTestId('artifacts')).toHaveTextContent('run-xyz');
  });

  it('clicking Artifacts tab sets aria-selected on Artifacts tab and removes it from Events', () => {
    render(<ExecutionView runId="run-1" reader={makeReader()} />);

    fireEvent.click(screen.getByRole('tab', { name: /^artifacts$/i }));

    expect(screen.getByRole('tab', { name: /^artifacts$/i })).toHaveAttribute('aria-selected', 'true');
    expect(screen.getByRole('tab', { name: /^events$/i })).toHaveAttribute('aria-selected', 'false');
  });
  ...
  it('auto-switches to Keywords tab when streamDone becomes true', async () => {
    mockUseRunStream.mockReturnValue(BASE_STREAM_STATE);
    const { rerender } = render(<ExecutionView runId="run-1" reader={makeReader()} />);
    ...
    // Stream completes
    mockUseRunStream.mockReturnValue(DONE_STREAM_STATE);
    rerender(<ExecutionView runId="run-1" reader={makeReader()} />);
  ...
  it('does not auto-switch back after user manually picks Artifacts', async () => {
    mockUseRunStream.mockReturnValue(DONE_STREAM_STATE);
    const { rerender } = render(<ExecutionView runId="run-1" reader={makeReader()} />);
    ...
    // User manually switches to Artifacts
    fireEvent.click(screen.getByRole('tab', { name: /^artifacts$/i }));
    expect(screen.getByRole('tab', { name: /^artifacts$/i })).toHaveAttribute('aria-selected', 'true');
  ...
  it('does not call onStreamDone while the stream is still running', () => {
    const onStreamDone = vi.fn();
    mockUseRunStream.mockReturnValue(BASE_STREAM_STATE);
    render(<ExecutionView runId="run-1" reader={makeReader()} onStreamDone={onStreamDone} />);
    expect(onStreamDone).not.toHaveBeenCalled();
  });

  it('calls onStreamDone once when the stream finishes', async () => {
    const onStreamDone = vi.fn();
    mockUseRunStream.mockReturnValue(BASE_STREAM_STATE);
    const { rerender } = render(
      <ExecutionView runId="run-1" reader={makeReader()} onStreamDone={onStreamDone} />,
    );
    expect(onStreamDone).not.toHaveBeenCalled();

    mockUseRunStream.mockReturnValue(DONE_STREAM_STATE);
    rerender(<ExecutionView runId="run-1" reader={makeReader()} onStreamDone={onStreamDone} />);
  ...
  it('shows "Stalled" status when the stream has stalled', () => {
    mockUseRunStream.mockReturnValue(STALLED_STREAM_STATE);
    render(<ExecutionView runId="run-1" reader={makeReader()} />);
  ...
  it('shows "Complete" (not "Stalled") once done even if stalled flag lingers', () => {
    mockUseRunStream.mockReturnValue({ ...STALLED_STREAM_STATE, streamDone: true });
    render(<ExecutionView runId="run-1" reader={makeReader()} />);
  ...
  it('ArrowRight moves focus from Clusters to Artifacts', () => {
    render(<ExecutionView runId="run-1" reader={makeReader()} />);
  ...
  it('ArrowRight wraps from Artifacts back to Commentary (idx 0)', () => {
    render(<ExecutionView runId="run-1" reader={makeReader()} />);
  ...
  it('ArrowLeft wraps from Commentary (idx 0) to Artifacts', () => {
    render(<ExecutionView runId="run-1" reader={makeReader()} />);
